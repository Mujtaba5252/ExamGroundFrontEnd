...
function SignIn() {
  ...
  const handleLogin=async ()=>{
    ...
    let TeacherData = await fetch("http://localhost:5000/SignIn", {
                    method: "post",
                    body: JSON.stringify({email,password,}),
                    headers: { "Content-Type": "application/json" },
                });
    let studentData = await fetch("http://localhost:5000/StudentSignIn", {
                  method: "post",
                  body: JSON.stringify({email,password,}),
                  headers: { "Content-Type": "application/json" },
              });

                const TeacherResult = await TeacherData.json();
                const studentResult = await studentData.json();
                console.log(TeacherResult);
                console.log(studentResult);
                if(TeacherResult.email)
                {
                  localStorage.setItem("user",JSON.stringify(TeacherResult));
                  navigate("/TeacherDashboard")
                
                }
                else if(studentResult.email)
                {
                  localStorage.setItem("user",JSON.stringify(studentResult));
